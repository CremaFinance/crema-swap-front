...
export async function addLiquidityNew(
  ...
  if (poolInfo.coin.mintAddress === NATIVE_SOL.mintAddress) {
    wrappedCoinSolAccount = await createTokenAccountIfNotExist(
      ...
      owner,
      WSOL.mintAddress,
      Number(coinAmount) > Number(coinBalance) ? Number(coinBalance) - 1000000 : Number(coinAmount),
      transaction,
      signers
    )
  }
  ...
  if (poolInfo.pc.mintAddress === NATIVE_SOL.mintAddress) {
    wrappedSolAccount = await createTokenAccountIfNotExist(
      ...
      owner,
      WSOL.mintAddress,
      Number(pcAmount) > Number(pcBalance) ? Number(pcBalance) - 1000000 : Number(pcAmount),
      transaction,
      signers
    )
  }
